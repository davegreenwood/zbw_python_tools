...
def animPullDownUI():
    ...
    widgets["zeroBut"] = cmds.button("zeroButton", l="Add Selected Master CTRLs", bgc = (.8,.8,.6), w=400, h=30, c= partial(getControl, "masterTSL"))
    ...
    cmds.separator(h=10)
    widgets["masterTSL"] = cmds.textScrollList("masterTSL", nr=4, w=400, h=60, ams=True, dcc=partial(showName, "masterTSL"), bgc=(.2, .2, .2))

    #ik items layout
    cmds.setParent(widgets["mainCLO"])
    widgets["IKFLO"] = cmds.frameLayout("IKFrameLO", l = "World Space Controls (Translation and Rotation)", w=400, bgc=(0,0,0))
    widgets["IKCLO"] = cmds.columnLayout("IKColumnLO", w=400)
    cmds.text("Select World Space Controls (translate and rotate)")
    widgets["IKBut"] = cmds.button("IKButton", l="Add World Space CTRLs", w=400, h=30, bgc = (.8,.8,.6), c= partial(getControl, "IKTSL"))
    ...
    widgets["IKStoredBut"] = cmds.button("storedIKButton", l="Add Stored From Selected Master", bgc=(.6,.6,.8), w=200, h=20, c= partial(addStoredSolo, "storeTSL", "IKTSL"))
    ...
    cmds.separator(h=10)
    widgets["IKTSL"] = cmds.textScrollList("IKTSL", nr=10, w=400, h=90, ams=True, dcc=partial(showName, "IKTSL"), bgc=(.2, .2, .2))
    cmds.separator(h=10)

    #ik items layout
    cmds.setParent(widgets["mainCLO"])
    widgets["rotFLO"] = cmds.frameLayout("rotFrameLO", l = "World Rotation Controls (Rotation Only) -optional-", w=400, bgc=(0,0,0), cll=True, cl=True)
    widgets["rotCLO"] = cmds.columnLayout("rotColumnLO", w=400)
    cmds.text("Select World Rotation Controls (rotation only!)")
    widgets["rotBut"] = cmds.button("rotButton", l="Add World Rotation CTRLs", w=400, h=30, bgc = (.8,.8,.6), c= partial(getControl, "rotTSL"))

    widgets["rotRCLO"] = cmds.rowColumnLayout("rotRCLO", nc=2, w=400)
    widgets["rotClearSelBut"] = cmds.button("clearrotButton", l="Clear Selected", bgc=(.8,.6,.6), w=200, h=20, c= partial(clearList, "rotTSL"))
    widgets["rotClearAllBut"] = cmds.button("moveRotButton", l="Clear All", w=200, h=20, bgc=(.8,.5,.5), c= partial(clearAll,  "rotTSL"))
    widgets["rotStoredBut"] = cmds.button("storedRotButton", l="Add Stored From Selected Master", bgc=(.6,.6,.8), w=200, h=20, c= partial(addStoredSolo, "storeRotTSL","rotTSL"))
    widgets["rotSelObjBut"] = cmds.button("selObjRotButton", l="Grab All Items From List", bgc=(.5,.6,.8), w=200, h=20, c= partial(selectObj, "rotTSL"))
    cmds.setParent("rotColumnLO")
    cmds.separator(h=10)
    widgets["rotTSL"] = cmds.textScrollList("rotTSL", nr=10, w=400, h=70, ams=True, dcc=partial(showName, "rotTSL"), bgc=(.2, .2, .2))
    cmds.separator(h=10)

    cmds.setParent(widgets["mainCLO"])
    #create key type rbuttons
    widgets["keyRBG"] = cmds.radioButtonGrp(nrb=3, l="Key Type:", l1="Step", l2="Auto", l3="linear", sl=3, cw=[(1,75), (2,60), (3,60), (4,60)], cal=[(1,"left"),(2,"left"),(3,"left"),(4,"left")])
    #doIt button layout
    cmds.setParent(widgets["mainCLO"])
#TO-DO----------------clear all button!
    widgets["doItRCLO"] = cmds.rowColumnLayout("doItLayout", nc=3, cw=[(1,220), (2,100),(3,80)])
    widgets["doItBut"] = cmds.button("doItButton", l="Pull Animation Down from Master!", w=220, h=50, bgc = (.4,.8,.4), c=pullDownAnim)
    widgets["pullBut"] = cmds.button("pullButton", l="Store \nWS Controls", w=100, h=50, bgc = (.4,.4,.8), c=storeControls)
    widgets["clearBut"] = cmds.button("clearButton", l="Clear All\nFields", w=80, h=50, bgc = (.8,.4,.4), c=clearAllLists)


    #create second tab
    cmds.setParent(widgets["tabLO"])
    widgets["storeCLO"] = cmds.columnLayout("Stored Control Names", w=400)
    widgets["storeFL"] = cmds.frameLayout(l="Stored World Space Control Names (Translate and Rotate)", w=400, bgc = (0,0,0))
    widgets["storeRCL"] = cmds.rowColumnLayout(nc=2)
    widgets["storeClearSelBut"] = cmds.button(l="Clear Selected", bgc = (.8,.6,.6), w=200, h=20, c=partial(clearList, "storeTSL"))
    widgets["storeClearAllBut"] = cmds.button(l="Clear All", bgc = (.9,.6,.6), w=200, h=20, c=partial(clearAll, "storeTSL"))
    ...
    widgets["storeRotFL"] = cmds.frameLayout(l="Stored World Rotation Control Names (Rotation Only!)", w=400, bgc = (0,0,0))
    widgets["storeRotRCL"] = cmds.rowColumnLayout(nc=2)
    widgets["storeRotClearSelBut"] = cmds.button(l="Clear Selected", bgc = (.8,.6,.6), w=200, h=20, c=partial(clearList, "storeRotTSL"))
    widgets["storeRotClearAllBut"] = cmds.button(l="Clear All", bgc = (.9,.6,.6), w=200, h=20, c=partial(clearAll, "storeRotTSL"))
    ...
    widgets["storeRotPullBut"] = cmds.button(l="Pull Controls To Store Lists from Previous Tab!", bgc = (.8,.8,.4), w=400, h=40, c=storeControls)
    widgets["storeRotPushBut"] = cmds.button(l="Push These To Lists from Selected Master Ctrl!", bgc = (.4,.8,.4), w=400, h=40, c=partial(addStored, "yup"))
    cmds.text("WARNING!\nThis 'push' button clears all fields on prev tab and add the stored controls \n to their respective lists AND put the selected master control into \nthe 'Master List'", al="center", w=400)
    ...
    #-------------
        ...
        for key in range(len(keyList)):
            ...
            for wCtrl in worldCtrls:
                ...
                cmds.keyTangent(wCtrl, at=["tx", "ty", "tz", "rx", "ry", "rz"], itt=inTan, ott=outTan, t=(keyList[key], keyList[key]))
            ...
            for rCtrl in rotateCtrls:
                #--------if attr is locked skip it
                cmds.xform(rCtrl, ws=True, ro=(localVals[rCtrl][key][3], localVals[rCtrl][key][4], localVals[rCtrl][key][5]))
                try:
            ...
            cmds.warning("You need at least one Master ctrl and either a World Space ctrl or World Rotation ctrl in the lists!")
